...
function handleResult(data) { // this is where the validation based on fetched result takes place... i.e, whether the code ran successfully, or had a compile error or runtime error etc.
...
function run(sourceValue, inputValue) {
  ...
  $.ajax({
    url: BASE_URL + `/submissions?base64_encoded=true&wait=${WAIT}`, //This is the api call. The wait parameter signifies it waits until the execution is complete and after that it will automatically send the token to get the results
    type: "POST",
    async: true,
    contentType: "application/json",
    data: JSON.stringify(data),
    success: function(data, textStatus, jqXHR) { // success, the token is gotten
...
// // Testing 
// function Testing(){
//   qNo = $selectQuestionBtn.val();
//   switch($selectLanguageBtn){
//     case 
